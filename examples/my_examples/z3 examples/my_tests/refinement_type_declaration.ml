...
let _ = 
  try (
	...
	let pi = (Real.mk_numeral_s ctx "3.14159") in
	let w = (Arithmetic.mk_mul ctx [(Integer.mk_numeral_s ctx "2.0");
	                                 pi]) in
      	let y0 = (Real.mk_numeral_s ctx "5.0") in
	let constraints = (Arithmetic.mk_ge ctx y0 pi) in
