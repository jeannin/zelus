...
let name ff n = shortname ff (Zident.name n)

let source_name ff n = shortname ff (Zident.source n)
...
let print_vardec_list ff vardec_list =
  let vardec ff
    ...
    fprintf ff "@[%a%a%a@]" name n
      (Zmisc.optional_unit default) d_opt (Zmisc.optional_unit combine) c_opt in
  if vardec_list <> []
...
let print_binding ff (n, { t_sort = sort; t_typ = typ }) =
  ...
  | Svar { v_combine = c_opt; v_default = d_opt } ->
     fprintf ff "@[var %a: %a%a%a@,@]" name n Ptypes.output typ
	     (Zmisc.optional_unit default) d_opt
	     (Zmisc.optional_unit combine) c_opt
  | Smem { m_kind = k; m_next = n_opt; m_previous = p;
	   m_init = i_opt; m_combine = c_opt } ->
     fprintf ff "@[%a%s%a mem %a: %a%a%a@,@]"
	     (Zmisc.optional_unit next) n_opt (previous p)
	     (Zmisc.optional_unit kind) k name n Ptypes.output typ
	     init i_opt
	     (Zmisc.optional_unit combine) c_opt
...
let print_writes ff { dv = dv; di = di; der = der; nv = nv; mv = mv } =
  if !vverbose then begin
    let dv = Zident.S.elements dv in
    let di = Zident.S.elements di in
    let der = Zident.S.elements der in
    let nv = Zident.S.elements nv in
    let mv = Zident.S.elements mv in
    open_box 0;
    if dv <> [] then
      fprintf ff
...
(* Debug printer for (Zident.t * Deftypes.typ) Zlmisc.State.t *)
let state_ident_typ =
  let fprint_v ff (id, ty) =
    fprintf ff "@[%a:%a@]" Zident.fprint_t id Ptypes.output ty in
  Zmisc.State.fprint_t fprint_v

(* Debug printer for Hybrid.eq Zmisc.State.t *)
let state_eq = Zmisc.State.fprint_t equation
...
and print_value ff ve =
  ...
  | Vfun(body, venv) ->
      fprintf ff "@[<hov0><%a,@,%a>@]"
        funexp body (Zident.Env.fprint_t print_value_code) venv
